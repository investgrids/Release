...
## B.5-C — NSE evidence coverage: diagnosis + historical-range feasibility (same evening)

Owner authorized a narrow follow-up given Gate 2's recall bottleneck traced to `EvidenceEntityLink` coverage (415 of 2,557 real entities have any linked NSE evidence): (1) measure *why* coverage is low before assuming a fix is needed, (2) if the diagnosis leaves an open question about historical retrieval, run a strictly read-only, non-aggressive live feasibility probe — not a backfill — against NSE's real public API.

### Diagnosis: NOT a resolution bug

Checked the real linking code (`app/services/warehouse/evidence_entity_linking.py`, reuses the same `resolve_identifier()` AI Search already uses — not a second scheme). **619 of 632 real NSE filings (97.9%) resolved successfully.** All 13 unresolved cases were individually traced against `CompanyEntity` by name/symbol: none exist under a different symbol (ruling out a normalization bug) — they're either real companies genuinely absent from Company Master (McNally Bharat, SKIL Infrastructure, Talwalkars, Winsome Yarns, Gammon India, Radha Madhav, AJR Infra, Nagarjuna Fertilizers & Chemicals — several with known real distressed/insolvency histories) or a different legal instrument entirely (IRB InvIT Fund — the underlying sponsor "IRB Infrastructure Developers" IS in Company Master, but the InvIT itself is a separate real legal form, correctly out of scope for a company-only master).

The real explanation for low entity-coverage: NSE ingestion (`app/providers/nse_provider.py`) only polls `fetch_latest()` — no `from_date`/`to_date` ever used in production — on a rolling window that has produced exactly the ~9 real days (Aug 21–30, 2026) seen in this corpus. No historical backfill has ever run. `fetch_by_date()` exists on the provider but is unused for this purpose, and the shared `_get()` helper hard-caps every response to 50 items regardless of range requested — a real, load-bearing detail that made a live feasibility check necessary before any backfill could be responsibly designed (a raw response of 327 silently sliced to 50 is a different, much more urgent bug than NSE itself only ever returning 50).

### Live feasibility probe (read-only, no backfill)

`scripts/b5c_nse_historical_probe.py` — standalone (imports only `httpx`/`asyncio`/`json`/`datetime`, no DB/app imports at all, so it cannot touch persistence even by accident). Tested 7 checkpoints (30 days, ~3/6 months, ~1/2 years back, one likely-high-volume day, one wide range) via the exact request shape (`_URL`, headers, warm-up-then-fetch pattern) already used by production's real, working ingestion — calling the raw endpoint directly (bypassing the app's 50-item slice) so any real NSE-side count would have been visible before truncation.

**Run 1 (local machine):** warm-up to NSE's homepage returned **HTTP 403** (Akamai bot-protection cookies, no valid session); all 7 subsequent calls returned HTTP 200 with a generic `"Missing index"` HTML error, not real data. Inconclusive — attributed to the local IP lacking standing with NSE's anti-bot layer.

**Run 2 (Railway production, via `railway ssh`):** same script, same single warm-up request. **Also returned HTTP 403.** Per the owner's explicit stop condition, the script halted immediately — zero retries, no header/cookie/proxy experimentation, only the one warm-up request was ever sent from production for this probe.

**Real observation, not acted on further**: production's own live scheduled NSE ingestion succeeded repeatedly in the same minutes as the blocked probe (real events processed at 17:16–17:22 UTC the same day) — so the block is not a blanket IP ban; it is specific to something about the probe's own request (most plausibly the `from_date`/`to_date` query parameters themselves, which production's real `fetch_latest()` calls never send). Investigating that distinction further would mean exactly the kind of experimentation the owner's stop condition rules out, so it is recorded here as a lead for a future, separately-authorized session, not pursued.

### Verdict: **RAILWAY ALSO BLOCKED → B.5-C historical-range feasibility: INCONCLUSIVE**

No evidence either way on whether NSE's real endpoint supports historical `from_date`/`to_date` ranges, whether pagination exists, or whether the 50-item cap is NSE-side or the app's own slicing — the access/session layer blocked the test before the API contract itself could be exercised, both locally and from production. The three real, load-bearing facts established this pass stand regardless: (1) resolution logic is not the bottleneck (97.9% success), (2) the small residual unresolved set is genuine absence from Company Master, not a bug, (3) the app's own `_get()` helper unconditionally truncates every response to 50 items, which is worth auditing separately since it could mean **current, ongoing ingestion is silently incomplete on any real high-volume day**, independent of the historical-backfill question.

## Conclusion

Gate 1's real, full-census precision moved from 77.3% (v3) to 99.2% (v4, 124/125). Gate 2 now exists as real, tested code, runs independently per entity (proven via a real mixed-verdict story), and — after 4 fixes traced to specific demonstrated failures, not guesses — has **7/7 hand-verified correct matches, zero known wrong-event matches**, at low but now better-understood recall (6/88 single, 1/37 multi). The dominant remaining constraint is real NSE evidence coverage (`EvidenceEntityLink`), not matching quality, and coverage is limited by ingestion depth/volume (confirmed: not a resolution bug) — but whether historical backfill is even feasible remains genuinely open after two blocked feasibility probes. B.5's next real lever is either a legitimate alternative historical NSE source, or an audit of the current 50-item response cap on *live* ingestion (a separate, more urgent question). **Still not ready for backfill.**
