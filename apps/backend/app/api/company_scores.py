...
from app.services.aipe.company_score_engine import (
    compute_company_score, compute_sector_rankings, get_sector_signal_counts,
    historical_track_record, sector_strength, get_ranking_stats,
)
...
async def get_stats(db: AsyncSession = Depends(get_db)):
    return await get_ranking_stats(db)
...
async def get_company_historical(symbol: str, db: AsyncSession = Depends(get_db)):
    return await historical_track_record(db, symbol)
...
@router.get("/sector/{sector}/strength")
async def get_sector_strength(sector: str, db: AsyncSession = Depends(get_db)):
    return await sector_strength(db, sector)
