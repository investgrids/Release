...
Opportunity), called fresh whenever a caller needs a window of evidence.
Follows the same "@dataclass, never invents evidence, degrades gracefully"
pattern already established by app/services/ai_search/evidence.py's
EvidenceBundle — reused, not reinvented.
...
materiality/synthesis/scoring layer that treats all of them as equally
trustworthy would be wrong. Every normalizer below states its
classification and why.

Moved here from app/services/weekend_intelligence/evidence.py in Phase
5E.3 — see this package's __init__.py for why. Content is unchanged;
weekend_intelligence/evidence.py now re-exports from here.
"""
