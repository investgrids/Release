...
One real-world development (e.g. an RBI rate decision, or a company's
NSE filing also covered by a news outlet) can surface as a NewsArticle
+ an Event + a CompanyAnnouncement + an AICompanySignal + an
Opportunity. Those must not count as independent confirmations. No LLM
is used here — clustering is deterministic, in two tiers:
...
Tier 1 — REAL relational links already in the schema, no heuristics:
  ...
  - AICompanySignal.source_id: normalize_company_signal() encodes this
    as f"{source_type}:{source_id}" (evidence.py). When source_type ==
    "opportunity", source_id is a real Opportunity.id — an exact link
    back to that Opportunity's own EvidenceItem.
...
Moved here from app/services/weekend_intelligence/dedup.py in Phase
5E.3 — see this package's __init__.py for why (this logic was never
Weekend-Intelligence-specific; only its import path was). Content
unchanged; weekend_intelligence/dedup.py now re-exports from here.
"""
...
class EvidenceCluster:
    ...
    def representative(self) -> EvidenceItem:
        ...
        from app.services.evidence_clustering.evidence import DETERMINISTIC
    ...
    def net_direction(self) -> str:
        ...
        silently picks a side) — feeds risk synthesis: a thesis with
        real contradictory evidence must not look uncontested."""
...
async def cluster_evidence(db: AsyncSession, evidence: list[EvidenceItem]) -> list[EvidenceCluster]:
    """
    Group `evidence` (already normalized, already window-bounded) into
    EvidenceClusters. Order of the returned clusters is not meaningful;
    ...
    # are small; quadratic over a few hundred items is fine, not worth a
    # smarter index for this data volume.
