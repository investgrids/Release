...
def _patched_common(session: str, price_map: dict[str, float]):
...
async def test_weekday_live_session_fetches_all_five_instruments():
    p1, p2, p3, p4, p5, bus_mock = _patched_common("live", _ALL_TICKERS)
    with p1, p2 as fetch_mock, p3, p4, p5:
...
async def test_weekday_after_close_skips_nse_instruments_fetches_continuous():
    p1, p2, p3, p4, p5, bus_mock = _patched_common("post_market", _ALL_TICKERS)
    with p1, p2 as fetch_mock, p3, p4, p5:
        await pm.run_price_monitor_cycle()
    fetched_tickers = {call.args[0] for call in fetch_mock.call_args_list}
...
async def test_weekday_pre_market_skips_nse_instruments_fetches_continuous():
    p1, p2, p3, p4, p5, bus_mock = _patched_common("pre_market", _ALL_TICKERS)
    with p1, p2 as fetch_mock, p3, p4, p5:
        await pm.run_price_monitor_cycle()
    fetched_tickers = {call.args[0] for call in fetch_mock.call_args_list}
...
async def test_weekend_fetches_zero_instruments_from_the_threshold_loop():
    """The locked correction: USDINR/BRENT must also be skipped on
    Sat/Sun, unlike weekday-closed hours."""
    p1, p2, p3, p4, p5, bus_mock = _patched_common("weekend", _ALL_TICKERS)
    with p1, p2 as fetch_mock, p3, p4 as close_mock, p5 as obs_mock:
        await pm.run_price_monitor_cycle()
...
async def test_capture_close_snapshot_and_market_observations_run_regardless_of_nse_session():
    ...
    for session in ("live", "pre_market", "post_market", "weekend"):
        p1, p2, p3, p4, p5, bus_mock = _patched_common(session, _ALL_TICKERS)
        with p1, p2, p3, p4 as close_mock, p5 as obs_mock:
            await pm.run_price_monitor_cycle()
        close_mock.assert_awaited_once()
        obs_mock.assert_awaited_once()
...
async def test_reopen_after_weekend_gap_compares_against_last_live_close_not_suppressed():
    ...
    p1, p2, p3, p4, p5, bus_mock = _patched_common("live", monday_prices)
    with p1, p2, p3, p4, p5:
        await pm.run_price_monitor_cycle()
