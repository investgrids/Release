...
export function WelcomeBackHero({ date, status, greeting, timeIST, stats }: WelcomeBackHeroProps) {
  const { history, lastViewed } = useRecentHistory();
  ...
  const kpis = [
    {
      ...
      value: stats.sentimentLabel, sub: `${stats.sentimentScore} / 100`,
      vs: `↑ ${stats.sentimentChange} vs yesterday`,
      color: "text-emerald-400", border: "border-emerald-500/20", glow: "from-emerald-500/10 to-transparent",
      icon: (
    ...
    {
      id: "events", label: "Events Today",
      value: String(stats.eventsToday), sub: "",
      vs: `↑ ${stats.eventsTodayVs} vs yesterday`,
      color: "text-sky-400", border: "border-sky-500/20", glow: "from-sky-500/10 to-transparent",
      icon: (
    ...
    {
      id: "highimpact", label: "High Impact Events",
      value: String(stats.highImpactEvents), sub: "",
      vs: `↑ ${stats.highImpactVs} vs yesterday`,
      color: "text-orange-400", border: "border-orange-500/25", glow: "from-orange-500/12 to-transparent",
      icon: (
    ...
    },
    {
      ...
      value: String(stats.opportunityScore), sub: "",
      vs: `↑ ${stats.opportunityVs} pts vs yesterday`,
      color: "text-violet-400", border: "border-violet-500/20", glow: "from-violet-500/10 to-transparent",
      icon: (
    ...
    },
    {
      ...
      value: `${stats.aiConfidence}%`, sub: stats.aiConfidenceLabel,
      vs: `↑ ${stats.aiConfidenceVs} vs yesterday`,
      color: "text-teal-400", border: "border-teal-500/20", glow: "from-teal-500/10 to-transparent",
      icon: (
  ...
  return (
    <div className="space-y-3">
          ...
          <div className="mb-5 flex items-start justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className={`text-[11px] ${open ? "text-emerald-400" : "text-rose-400"}`}>{status}</span>
              </div>
            </div>
          </div>
      ...
      {/* Continue Research — rendered below the hero card if there's history */}
      {(recentItems.length > 0 || history.searches.length > 0) && (
        <ContinueResearch items={recentItems} searches={history.searches} />
      )}
    </div>
  );
}
