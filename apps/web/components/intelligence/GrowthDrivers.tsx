...
const IMPACT_STYLES = {
  ...
  low:    { pill: "border-surface-border/7  bg-slate-500/10  text-text-secondary",    dot: "bg-slate-500"   },
};
  ...
  return (
    <div className="rounded-[20px] border border-surface-border/7 bg-text-primary/[0.025] p-5">
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-[10px] font-bold uppercase tracking-[0.12em] text-text-muted">Growth Drivers</span>
      </div>
        ...
        <div className="space-y-3">
          {drivers.map((d, i) => {
            ...
            return (
              <div
                key={i}
                className="rounded-[16px] border border-surface-border/6 bg-text-primary/[0.02] p-4 hover:border-emerald-400/20 transition-all"
              >
                <div className="flex items-start justify-between gap-3 mb-1.5">
                  <div className="flex items-center gap-2">
                    <span className={`h-2 w-2 shrink-0 rounded-full ${style.dot}`} />
                    <p className="text-[13px] font-semibold text-text-primary">{d.title}</p>
                  </div>
                  <span className={`shrink-0 rounded-full border px-2 py-0.5 text-[10px] font-bold capitalize ${style.pill}`}>
                    {d.impact}
                  </span>
                </div>
                <p className="text-[12px] text-text-secondary leading-5 ml-4 mb-2">{d.description}</p>
                <div className="ml-4">
