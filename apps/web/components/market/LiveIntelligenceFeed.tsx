...
export function LiveIntelligenceFeed({ compact = false, limit = 10 }: { compact?: boolean; limit?: number }) {
  ...
  return (
    <div className={`rounded-2xl border border-white/[0.07] bg-[#080c14] ${compact ? "p-4" : "p-5"}`}>
      ...
      {entries.length === 0 ? (
        ...
        <div className={`space-y-2 overflow-y-auto pr-1 ${compact ? "max-h-[360px]" : "max-h-[520px]"}`}>
          {entries.map(entry => (
