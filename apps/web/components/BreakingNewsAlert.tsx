...
const URGENCY_BADGE = {
  critical: "bg-rose-500/20 text-rose-600 dark:text-rose-300 border border-rose-500/30",
  high:     "bg-amber-500/15 text-amber-600 dark:text-amber-300 border border-amber-500/30",
};
...
function AlertCard({ alert, onDismiss }: { alert: BreakingAlert; onDismiss: () => void }) {
  ...
  return (
    <div className={`relative w-full overflow-hidden rounded-[20px] border bg-surface-card/95 backdrop-blur-2xl transition-all duration-300 ${URGENCY_STYLE[urgency]}`}>
      ...
      <div className="flex items-start gap-3 p-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-sm font-semibold text-text-primary leading-snug line-clamp-2">{alert.headline}</p>
        </div>

        {/* Controls */}
        <div className="flex shrink-0 items-center gap-1">
          <button onClick={() => setExpanded(e => !e)}
            className="flex h-7 w-7 items-center justify-center rounded-full bg-text-primary/5 text-text-secondary hover:text-text-primary transition text-xs">
            {expanded ? "▲" : "▼"}
          </button>
          <button onClick={onDismiss}
            className="flex h-7 w-7 items-center justify-center rounded-full bg-text-primary/5 text-text-secondary hover:text-text-primary transition">
      ...
      {/* Expanded body */}
      {expanded && (
        <div className="border-t border-surface-border/8 px-4 pb-4 pt-3 space-y-3">
          {/* AI Summary */}
          <p className="text-xs leading-5 text-text-secondary">{alert.summary}</p>
                ...
                {alert.stocks.map((s, i) => (
                  <div key={i} className={`flex items-center gap-2.5 rounded-[10px] px-3 py-2 ${
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[10px] text-text-secondary leading-4 truncate">{s.reason}</p>
                    </div>
                    <span className={`shrink-0 text-[10px] font-medium capitalize ${
                      s.magnitude === "high" ? "text-rose-600 dark:text-rose-300" : s.magnitude === "medium" ? "text-amber-600 dark:text-amber-300" : "text-text-secondary"
                    }`}>{s.magnitude}</span>
                  </div>
                ))}
              </div>
          ...
          {alert.sectors?.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {alert.sectors.map(s => (
                <span key={s} className="rounded-full bg-text-primary/5 border border-surface-border/8 px-2.5 py-0.5 text-[10px] text-text-secondary">
                  {s}
                </span>
              ))}
            </div>
          ...
          <div className="flex items-center gap-2 pt-1">
            ...
            <button onClick={onDismiss}
              className="rounded-[12px] border border-surface-border/10 bg-text-primary/5 px-3 py-2 text-xs text-text-secondary hover:text-text-primary hover:bg-text-primary/10 transition">
              Dismiss
            </button>
          </div>
        </div>
