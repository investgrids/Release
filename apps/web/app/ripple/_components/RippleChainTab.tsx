...
// Recovered/reused verbatim from the former standalone app/ripple/page.tsx
// (now the hub shell) — same fetch, same cards, same NextSteps guidance.
// metadata/dynamic exports removed (only valid on the actual route file,
// now app/ripple/page.tsx).
...
export async function RippleChainTab() {
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {([
          ...
          <div key={s.step} className="rounded-xl border border-surface-border/6 bg-surface-card p-4">
            ...
            <p className="text-[12px] font-bold text-text-primary mb-1">{s.title}</p>
            <p className="text-[11px] text-text-muted leading-5">{s.desc}</p>
          </div>
        ))}
      </div>
      ...
      <div>
        <div className="mb-4 flex items-center justify-between">
          <div>
            <h2 className="text-[18px] font-bold text-text-primary">Today's Biggest Ripples</h2>
            <p className="text-[12px] text-text-muted mt-0.5">Click any event to launch the full Ripple Engine analysis</p>
          </div>
        ...
        {displayEvents.length === 0 ? (
          <div className="rounded-xl border border-surface-border/7 bg-surface-card py-16 text-center">
            <p className="text-sm text-text-muted">No scored ripple events right now — check back shortly.</p>
          </div>
        ) : (
              ...
              >
                ...
                <div className="flex items-start justify-between gap-3 mb-3">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    {!unscored && <span className="text-[9px] text-text-muted">/100</span>}
                  </div>
                </div>
                {/* Title */}
                <h3 className="text-[13px] font-bold text-text-primary leading-snug mb-2 group-hover:text-indigo-300 transition-colors line-clamp-2">
                  {ev.title}
                </h3>
                {/* Summary */}
                <p className="text-[11px] text-text-muted leading-5 line-clamp-2 mb-3">
                ...
                {cats.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {cats.slice(0, 3).map((c: string) => (
                      <span key={c} className="rounded border border-surface-border/6 bg-text-primary/[0.03] px-2 py-0.5 text-[9px] text-text-muted">
                        {c}
                      </span>
                    ))}
                  </div>
                ...
                <div className="mt-3 flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-text-muted group-hover:text-indigo-400 transition-colors">
                    View Ripple Engine →
                  </span>
                </div>
              </Link>
      ...
      <div className="rounded-xl border border-surface-border/6 bg-surface-card p-5">
        <div className="grid grid-cols-2 sm:grid-cols-4 divide-x divide-surface-border/5">
          {[
            ...
            <div key={s.label} className="px-6 first:pl-0 last:pr-0">
              ...
              <p className="text-[11px] font-semibold text-text-secondary mt-0.5">{s.label}</p>
