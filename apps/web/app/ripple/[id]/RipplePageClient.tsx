...
// ── Main page ─────────────────────────────────────────────────────────────────
// initialData/initialRelated (optional) come from the server-rendered
// wrapper (page.tsx), which fetches the same /api/ripple/event/{id} +
// /api/related/ripple/{id} endpoints server-side purely so crawlers and
// the first paint see real content instead of a loading skeleton — same
// pattern as CompanyPageClient.tsx.
export default function RipplePage({ initialData, initialRelated }: { initialData?: RippleData | null; initialRelated?: Record<string, RelatedItem[]> | null } = {}) {
  const { id } = useParams<{ id: string }>();
  const [data, setData] = useState<RippleData | null>(initialData ?? null);
  const [originalData, setOriginalData] = useState<RippleData | null>(initialData ?? null);
