...
function Card({ it }: { it: ComparisonItem }) {
  ...
  return (
    <Link href={`/research/${it.slug}`}
      className="group flex items-center justify-between rounded-[16px] border border-surface-border/8 bg-surface-card px-5 py-3.5 transition hover:-translate-y-0.5 hover:border-violet-500/25">
      <span className="text-[13.5px] font-bold text-text-primary">
        {a?.symbol ?? "?"} <span className="text-text-muted font-medium">vs</span> {b?.symbol ?? "?"}
      </span>
  ...
  return (
    <main className="mx-auto max-w-[1100px] space-y-8 px-6 py-6 pb-16">
      <nav className="flex items-center gap-2 text-[12px] text-text-muted">
        <Link href="/research" className="hover:text-text-secondary transition">Research</Link>
        <span>/</span>
      ...
      <div>
        ...
        <h1 className="mt-2 text-3xl font-black tracking-tight text-text-primary">AI Company Comparisons</h1>
      ...
      {items.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-[24px] border border-surface-border/8 bg-text-primary/[0.02] py-20 text-center">
          <p className="text-base font-semibold text-text-primary">No comparisons published yet</p>
          <p className="mt-1 text-sm text-text-muted">
            Try <Link href="/ai-search" className="text-violet-400 hover:text-violet-600 dark:text-violet-300">AI Search</Link> for a live, personalized comparison.
          </p>
        </div>
      ) : (
        <>
          {trending.length > 0 && (
            <section>
              <h2 className="mb-3 text-[13px] font-bold uppercase tracking-wide text-text-secondary">🔥 Trending</h2>
          ...
          {sectors.length > 0 && (
            <section>
              <h2 className="mb-3 text-[13px] font-bold uppercase tracking-wide text-text-secondary">Browse by Sector</h2>
              <div className="space-y-5">
                {sectors.map(([sector, list]) => (
                  <div key={sector}>
                    <p className="mb-2 text-[12px] font-semibold text-text-secondary">{sector} <span className="text-text-muted">({list.length})</span></p>
