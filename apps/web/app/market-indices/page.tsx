...
// MarketIndicesCharts.tsx's own header comment for why.
const MiniArea     = dynamic(() => import("./MarketIndicesCharts").then(m => m.MiniArea),     { ssr: false });
const HeroAreaChart = dynamic(() => import("./MarketIndicesCharts").then(m => m.HeroAreaChart), { ssr: false });
...
function IndexCard({ idx }: { idx: IndexQuote }) {
  ...
  return (
    <div className="rounded-[20px] border border-surface-border/10 bg-text-primary/[0.03] p-5 transition hover:-translate-y-0.5 hover:border-surface-border/20">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="mt-0.5 truncate text-sm font-semibold text-text-primary">{idx.name}</p>
          {ctx && <p className="text-[11px] text-text-muted mt-0.5">{ctx.desc}</p>}
        </div>
        <span className={`shrink-0 rounded-full px-2.5 py-0.5 text-[11px] font-medium ${idx.positive ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-300" : "bg-rose-500/10 text-rose-600 dark:text-rose-300"}`}>
          {idx.pct > 0 ? "+" : ""}{idx.pct.toFixed(2)}%
        </span>
      </div>
      <p className="mt-3 text-2xl font-bold text-text-primary">{idx.value}</p>
      ...
      <div className="mt-3 flex justify-between border-t border-surface-border/5 pt-3 text-[11px] text-text-muted">
        <span>H: <span className="text-text-secondary">{idx.high}</span></span>
        <span>L: <span className="text-text-secondary">{idx.low}</span></span>
      </div>
    </div>
  );
}
        ...
        {insights.map((ins, i) => (
          <div key={i} className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-[12px] leading-5 text-text-secondary">{ins.detail}</p>
            </div>
          </div>
        ))}
      </div>
      <div className="mt-4 flex flex-wrap items-center gap-3 border-t border-surface-border/5 pt-4">
        <Link
          href="/ai-search?q=What+do+today%27s+market+index+moves+mean+for+my+portfolio"
          className="inline-flex items-center gap-1.5 rounded-xl bg-violet-600/80 px-4 py-2 text-[12px] font-bold text-text-primary transition hover:bg-violet-500"
        >
          Ask AI what this means for me →
        </Link>
        <Link href="/events" className="text-[12px] font-medium text-sky-400 transition hover:text-sky-600 dark:text-sky-300">
          See what's driving this →
        </Link>
        <Link href="/market-intelligence" className="text-[12px] font-medium text-text-secondary transition hover:text-text-secondary">
          Live Market →
        </Link>
      </div>
    </div>
...
export default function MarketIndicesPage() {
  ...
  return (
    <main className="min-w-0 space-y-6 pb-10">
      <MarketContextStrip />
      <div>
        <p className="text-sm uppercase tracking-[0.24em] text-sky-600 dark:text-sky-300">Market Overview</p>
        <h1 className="mt-2 text-4xl font-semibold tracking-tight text-text-primary">Market Indices</h1>
        <p className="mt-1 text-sm text-text-secondary">An index tracks a group of stocks together. If Nifty 50 is up, most large companies are rising.</p>
      </div>
      ...
      {/* Hero spotlight */}
      {hero && (
        <div className="rounded-[24px] border border-surface-border/10 bg-text-primary/[0.03] p-6 shadow-glow">
          <div className="flex flex-wrap items-start justify-between gap-6">
            <div>
              <p className="text-xs uppercase tracking-widest text-text-muted">Benchmark</p>
              <p className="mt-1 text-xl font-bold text-text-primary">{hero.name}</p>
              <p className="mt-3 text-5xl font-semibold tracking-tight text-text-primary">{hero.value}</p>
            ...
            <div className="flex gap-8 text-sm">
              {[["Day High", hero.high], ["Day Low", hero.low]].map(([label, val]) => (
                <div key={label} className="text-center">
                  ...
                  <p className="mt-1 text-xl font-semibold text-text-primary">{val}</p>
                </div>
              ))}
              <div className="text-center">
                <p className="text-xs uppercase tracking-widest text-text-muted">Status</p>
                <span className="mt-1 inline-flex items-center gap-1.5 rounded-full bg-emerald-500/10 px-3 py-1 text-xs font-medium text-emerald-600 dark:text-emerald-300">
          ...
          {hero.chart.length > 0 && (
            <div className="mt-6 h-20">
              <HeroAreaChart chart={hero.chart} />
            </div>
          )}
        </div>
      )}
      ...
      {mainIndices.length > 0 && (
        <div>
          <p className="mb-3 text-[11px] font-bold uppercase tracking-[0.15em] text-text-muted">The Big Two — Watch these first</p>
      ...
      {vixIndex.length > 0 && (
        <div>
          <p className="mb-3 text-[11px] font-bold uppercase tracking-[0.15em] text-text-muted">Market Nervousness</p>
          <div className="grid gap-4 sm:grid-cols-2">
            {vixIndex.map((idx) => (
              <IndexCard key={idx.name} idx={idx} />
            ))}
      ...
      {sectorIndices.length > 0 && (
        <div>
          <p className="mb-3 text-[11px] font-bold uppercase tracking-[0.15em] text-text-muted">Sector Indices — Drill into specific industries</p>
      ...
      <p className="text-center text-[11px] text-text-muted">
        ...
        <span className="text-text-muted">NSE Live API / Upstox / Zerodha Kite</span>
      </p>
    </main>
  );
}
