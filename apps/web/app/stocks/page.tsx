...
export default function StocksPage() {
  ...
  // Fetch live prices for all stocks in parallel (single state update when all resolve)
  useEffect(() => {
    Promise.all(
    ...
    <main className="min-w-0 space-y-6 pb-10">
      ...
      <form onSubmit={handleSearch} className="relative">
        <div className="flex items-center overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04] shadow-glow transition focus-within:border-sky-500/40 focus-within:bg-sky-500/[0.02]">
          <Search className="ml-5 h-4 w-4 shrink-0 text-slate-500" />
          <input
            type="text"
            value={query}
      ...
      {filtered.length === 0 && (
        <div className="flex flex-col items-center gap-3 rounded-[20px] border border-white/10 bg-white/[0.03] py-16 text-center">
          ...
          <p className="text-sm text-slate-400">No companies found for &quot;{query}&quot;</p>
