...
// ── Design tokens ─────────────────────────────────────────────────────────────
const CARD = "rounded-[28px] border border-white/10 bg-slate-900/80 shadow-[0_20px_60px_rgba(0,0,0,.35)] transition-all duration-300 hover:border-sky-400/20";
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-4">
            <AITransparencyPanel
              confidence={72}
              reasoning={stock.description || "AI-generated analysis based on recent events, financials, and sector data."}
              companies={[{ name: stock.name, symbol: stock.symbol }]}
            />
            <AIDisclaimer />
          </div>
        </div>
              ...
              <tr key={r.symbol} className={`hover:bg-white/[0.02] transition ${r.isSelf ? "bg-sky-500/[0.04]" : ""}`}>
                <td className="py-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <Link href={`/stocks/${r.symbol}`} className={`font-semibold hover:text-sky-300 transition ${r.isSelf ? "text-sky-300" : "text-white"}`}>{r.symbol}</Link>
                ...
                <td className="py-3 text-right">
                  {!r.isSelf && <Link href={`/stocks/${r.symbol}`} className="text-[10px] text-sky-400 hover:text-sky-300 transition">View →</Link>}
                </td>
              </tr>
            ))}
          </tbody>
...
export default function StockPage({ params }: PageProps) {
  ...
  return (
    <main className="min-w-0 pb-16">
      <TrackPageVisit type="company" id={symbol.toUpperCase()} title={stock.name ?? symbol.toUpperCase()} subtitle={`${stock.price} · ${stock.sector}`} href={`/stocks/${symbol.toUpperCase()}`} />
            ...
            {renderGroup >= 2 ? <>
              ...
              <MultiHorizonOutlookCard
                fetchContext={{
                  ...
                  title:      stock.name,
                  symbol:     stock.symbol,
                  context:    stock.description,
                  sectors:    stock.sector ? [stock.sector] : [],
                  context_id: `stock:${stock.symbol}`,
                }}
              />
            </> : renderGroup >= 1 && <>
              <SectionSkel h={260}/>
