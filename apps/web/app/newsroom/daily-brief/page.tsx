  ...
  return (
    <div className="mx-auto max-w-4xl space-y-10 px-4 py-8 sm:px-6">
      ...
      <section>
        ...
        <h1 className="mt-3 text-[26px] font-black leading-tight text-text-primary md:text-[30px]">
          {cleanText(morning.headline)}
        </h1>
        {morning.published_at && (
          <p className="mt-1.5 text-[12px] text-text-muted">Published {fmtTime(morning.published_at)}</p>
        )}
        {morning.key_takeaway && (
          <p className="mt-4 rounded-xl border border-amber-500/20 bg-amber-500/[0.06] p-4 text-[14px] leading-6 text-amber-800/90 dark:text-amber-100/90">
            {cleanText(morning.key_takeaway)}
          </p>
        )}
        {morning.executive_summary && (
          <p className="mt-4 text-[14.5px] leading-7 text-text-secondary">{cleanText(morning.executive_summary)}</p>
        )}
      </section>
      ...
      {morning.what_happened && (
        <section>
          ...
          <p className="mt-3 text-[14.5px] leading-7 text-text-secondary">{cleanText(morning.what_happened)}</p>
        </section>
      )}
      ...
      {wrap && (
        <section>
          ...
          >
            <p className="text-[14.5px] font-semibold text-text-primary group-hover:text-sky-700 dark:text-sky-200">{cleanText(wrap.headline)}</p>
            {wrap.key_takeaway && (
              <p className="mt-1.5 line-clamp-2 text-[13px] leading-5 text-text-secondary">{cleanText(wrap.key_takeaway)}</p>
            )}
      ...
      <section>
        ...
        {events.length === 0 ? (
          ...
          <div className="mt-3 space-y-2">
            {events.map((e) => (
              <div key={e.id} className="flex items-center justify-between rounded-xl border border-surface-border/7 bg-text-primary/[0.03] p-3.5">
                <p className="text-[13.5px] text-text-primary">{cleanText(e.title)}</p>
                <span className="shrink-0 text-[11px] text-text-muted">{e.category} · {Math.round(e.impact_score)}</span>
              </div>
            ))}
          </div>
        )}
            ...
            {morning.opportunities.map((o, i) => (
              <div key={i} className="rounded-xl border border-emerald-500/15 bg-emerald-500/[0.04] p-4">
                <div className="flex items-center justify-between gap-2">
                  <p className="text-[13.5px] font-semibold text-emerald-700 dark:text-emerald-200">{cleanText(o.title)}</p>
                  {o.risk && (
                ...
                <p className="mt-1.5 text-[13px] leading-5 text-text-secondary">{cleanText(o.description)}</p>
                {o.timeframe && <p className="mt-1.5 text-[11px] text-text-muted">Timeframe: {o.timeframe}</p>}
              </div>
            ))}
          </div>
        </section>
          ...
          <div className="mt-3 space-y-3">
            {morning.risks.map((r, i) => (
              <div key={i} className="rounded-xl border border-rose-500/15 bg-rose-500/[0.04] p-4">
                <div className="flex items-center justify-between gap-2">
                  <p className="text-[13.5px] font-semibold text-rose-700 dark:text-rose-200">{cleanText(r.title)}</p>
                  {r.severity && (
                ...
                <p className="mt-1.5 text-[13px] leading-5 text-text-secondary">{cleanText(r.description)}</p>
                {r.mitigation && (
                  <p className="mt-1.5 text-[11.5px] leading-5 text-text-muted">
                    <span className="font-semibold text-text-secondary">Mitigation:</span> {cleanText(r.mitigation)}
                  </p>
                )}
              </div>
            ))}
        ...
        <section>
          ...
          <ul className="mt-3 space-y-2">
            {morning.what_to_watch_next.map((w, i) => (
              <li key={i} className="flex gap-2 text-[13.5px] leading-6 text-text-secondary">
      ...
      {morning.sources?.length > 0 && (
        <section className="border-t border-surface-border/7 pt-6">
          <p className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.18em] text-text-muted">
            <BadgeCheck className="h-3 w-3 text-text-secondary" /> Supporting Evidence
          </p>
          <div className="mt-3 flex flex-wrap gap-2">
            {morning.sources.map((s, i) => (
              <span key={i} className="rounded-full border border-surface-border/10 bg-text-primary/5 px-3 py-1 text-[11.5px] text-text-secondary">
                ✓ {s}
              </span>
            ))}
          </div>
