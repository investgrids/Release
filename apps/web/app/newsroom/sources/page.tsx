    ...
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
      <div className="mb-8">
        <p className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.18em] text-text-muted">
          <Clock className="h-3 w-3 text-text-secondary" /> AI Newsroom
        </p>
      ...
      {news.length === 0 ? (
        ...
        // does by default.
        <ul className="divide-y divide-surface-border/6 rounded-xl border border-surface-border/7 bg-text-primary/[0.02]">
          {news.map((n) => (
            <li key={n.id} className="flex items-start justify-between gap-4 px-5 py-3.5">
              <div className="min-w-0">
                <p className="text-[13.5px] font-medium leading-5 text-text-primary">{cleanText(n.headline)}</p>
                {n.summary && (
                  <p className="mt-1 line-clamp-1 text-[12px] leading-5 text-text-muted">{cleanText(n.summary)}</p>
                )}
                {n.sectors?.length > 0 && (
                  <div className="mt-1.5 flex flex-wrap gap-1">
                    {n.sectors.slice(0, 3).map((s, i) => (
                      <span key={i} className="rounded-full border border-surface-border/10 bg-text-primary/5 px-1.5 py-0.5 text-[10px] text-text-muted">
                        {s}
                      </span>
                    ))}
                  </div>
