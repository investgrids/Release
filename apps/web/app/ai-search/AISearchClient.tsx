...
function EmptyState({ onSearch }: { onSearch: (q: string) => void }) {
  return (
    <div className="flex flex-col items-center py-14 text-center space-y-8">
      ...
      <div>
        {/* SEO fix (deferred from the original audit): the page had no real
            <h1> on its default/crawled state — both existing h1s in
            SearchResults only render after a search runs, using
            result.query as text, which a crawler never sees on the bare
            URL. EmptyState and SearchResults are mutually exclusive
            renders (see the ternary below), so this is always the only h1
            present, never a duplicate. The bare URL now reaches this h1 in
            the raw server HTML too — see page.tsx, which forces this route
            to render dynamically per-request instead of statically, the
            actual fix for the crawler-visibility gap. */}
