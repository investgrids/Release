...
// ── Page ──────────────────────────────────────────────────────────────────────
// initialArticle (optional) comes from the server-rendered wrapper
// (page.tsx), which fetches the same /api/news/{id} endpoint server-side
// purely so crawlers and the first paint see real content instead of a
// loading skeleton.
export default function NewsDetailPage({ initialArticle }: { initialArticle?: NewsArticle | null } = {}) {
  ...
  const skippedFirstResetRef = useRef(!!initialArticle);
