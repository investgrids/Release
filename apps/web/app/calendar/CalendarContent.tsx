...
async function getPolicyEvents() {
  try {
    const events = await fetchAPI<PolicyEvent[]>("/api/events/?limit=50");
    return Array.isArray(events)
      ? events.filter(e => ["Government", "Policy", "RBI", "Macro"].includes(e.category))
      : [];
  } catch { return [] as PolicyEvent[]; }
}

async function getPolicyIntelligence() {
  try { return await fetchAPI<IntelligenceObject>("/api/intelligence/theme/government-policy-rbi"); }
  catch { return null; }
}
...
export async function CalendarContent({ headingLevel = "h1" }: { headingLevel?: "h1" | "h2" }) {
  const Heading = headingLevel;
  const [events, policyEvents, intelligence] = await Promise.all([
    getCalendar(),
    getPolicyEvents(),
  ...
  return (
    <main className="min-w-0 space-y-10 pb-10">
      ...
      <div>
        ...
        <Heading className="mt-2 text-4xl font-semibold tracking-tight text-text-primary">Policy & Calendar</Heading>
                      ...
                      <article key={e.id}
                        ...
                        <div className="flex items-stretch">
                          ...
                          <div className="flex min-w-0 flex-1 flex-col justify-center gap-1.5 p-4">
                            ...
                            <a href={`/ai-search?q=${encodeURIComponent(truncateForQuery(e.title))}`}
      ...
      <section aria-labelledby="recent-policy-events">
        ...
        <RecentPolicyEvents events={policyEvents} />
      </section>
      ...
      <div className="rounded-[20px] border border-amber-500/20 bg-amber-500/[0.04] p-4">
        <p className="text-xs text-amber-600 dark:text-amber-300">
          ...
          <strong>PIB (Press Information Bureau) API</strong>, and an{" "}
          <strong>Economic Calendar provider</strong> (e.g. Investing.com or Financial Modeling Prep).
        </p>
      </div>
    </main>
  );
