...
const CAT_CONFIG: Record<string, { color: string; icon: ReactNode }> = {
  RBI:     { color: "border-indigo-500/30 bg-indigo-500/10 text-indigo-300", icon: <Landmark className="h-3 w-3" /> },
  PMI:     { color: "border-sky-500/30 bg-sky-500/10 text-sky-300",         icon: <BarChart2 className="h-3 w-3" /> },
  GDP:     { color: "border-emerald-500/30 bg-emerald-500/10 text-emerald-300", icon: <TrendingUp className="h-3 w-3" /> },
  FX:      { color: "border-amber-500/30 bg-amber-500/10 text-amber-300",   icon: <ArrowLeftRight className="h-3 w-3" /> },
  Results: { color: "border-violet-500/30 bg-violet-500/10 text-violet-300", icon: <ClipboardList className="h-3 w-3" /> },
  Policy:  { color: "border-rose-500/30 bg-rose-500/10 text-rose-300",      icon: <ScrollText className="h-3 w-3" /> },
};
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          {Object.entries(CAT_CONFIG).map(([cat, cfg]) => (
            <span key={cat} className={`inline-flex items-center gap-1 rounded-full border px-3 py-1 text-xs font-medium ${cfg.color}`}>
              {cfg.icon} {cat}
            </span>
          ))}
        </div>
