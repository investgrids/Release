  ...
  if (error || !detail) return (
    <div className="min-w-0 pb-12">
      ...
      <div className="rounded-[20px] border border-rose-500/20 bg-rose-500/5 p-8 text-center">
        ...
        <Link href="/radar" className="mt-4 inline-block rounded-xl bg-white/10 px-4 py-2 text-[13px] text-white hover:bg-white/15 transition">← Back to Radar</Link>
      </div>
    </div>
  );
                    ...
                    <tbody className="divide-y divide-white/5">
                      {d.companies.map((c, i) => (
                        <tr key={i} className="hover:bg-white/[0.02] transition">
                          <td className="py-2.5">
                            <Link href={`/stocks/${c.symbol}`} className="flex items-center gap-2 hover:text-sky-400 transition">
